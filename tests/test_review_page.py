...
run("submitResponse('approval')");
assert.equal(run('isReviewed(DATA.groups[0])'), true);
run('active=0;DATA.groups[0].proposal=null;DATA.groups[0].proposal_id=null;DATA.groups[0].unresolved={person_ids:[],record_ids:[]}');
run("submitResponse('approval')");
assert.equal(run('isReviewed(group())'), false, 'nothing to arrange cannot be approved');
run("group().proposal={clusters:[],question:'Who is this?'};group().proposal_id='empty'");
run("submitResponse('approval')");
assert.equal(run('isReviewed(group())'), false, 'empty proposal with no evidence cannot be approved');
// arrangement: unresolved entries default to separate people; drag moves and merges carry into the approval
run('active=1;DATA.groups[1].proposal_id="changed"');
...
run("moveItem('r','person-1','ignore')");
assert.deepEqual(run('JSON.stringify(arrangementFor(group()).ignored)'), JSON.stringify([{kind:'r',id:'person-1'}]));
run("writeResponse('correction','',false)");
run("submitResponse('approval')");
assert.equal(run('isReviewed(DATA.groups[1])'), true, 'an arrangement is approvable');
assert.equal(run('state.responses[DATA.groups[1].key].arrangement.changed'), true);
run('active=0');
run('navigate(1)');
...
assert.equal(result.groups[1].response.type, 'approval');
assert.equal(result.groups[1].arrangement.ignored[0].id, 'person-1');

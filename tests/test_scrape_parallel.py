...
def test_four_tabs_share_queue_budget_and_serialize_writes(tmp_path, mocker):
    records = [{"id": f"instagram:u{i}", "handle": f"u{i}"} for i in range(12)]
    mocker.patch.object(run, "_select_records", return_value=records)
    mocker.patch.object(pace.random, "uniform", return_value=0)
    mocker.patch.dict("os.environ", {pace.DAILY_CAPS_ENV: '{"instagram": 8}'})
    ...
    result = run.scrape(
        "instagram", targets=["a", "b", "c", "d"], state_path=str(tmp_path / "state.json")
    )
    assert result == {"done": 8, "skipped": 0, "halted": "daily cap reached"}
    assert sorted(targets) == ["a", "b", "c", "d"]
    assert len(seen) == len(set(seen)) == 8
    state = json.loads((tmp_path / "state.json").read_text())["instagram"]
    assert next(iter(state.values()))["attempts"] == 8


def test_one_block_cancels_other_tabs_and_keeps_queue_pending(tmp_path, mocker):
...
@pytest.mark.parametrize("targets", [["a", "a"], [""], ["a", "b", "c", "d", "e"]])
...
def test_four_tabs_divide_only_gap_not_the_global_break(tmp_path, mocker):
    p = pace.Pacer("instagram", str(tmp_path / "state.json"))
    mocker.patch.object(pace.random, "uniform", return_value=8)
    for _ in range(24):
        assert p.next_gap(workers=4) == 2
    mocker.patch.object(pace.random, "uniform", side_effect=[8, 200])
    assert p.next_gap(workers=4) == 202
